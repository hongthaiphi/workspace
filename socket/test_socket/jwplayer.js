"undefined" == typeof jwplayer && (jwplayer = function (f) {
		...
		b.getAbsolutePath = function (h, a) {
			...
			if (b.exists(h)) {
				...
				if (b.exists(h)) {
					...
					var c = h.indexOf("?");
					d = 0 < d && (0 > c || c > d)
				} else d = void 0; if (d) return h;
				d = a.substring(0, a.indexOf("://") + 3);
				var c = a.substring(d.length, a.indexOf("/", d.length + 1)),
					e;
		...
		b.translateEventResponse = function (a, d) {
			var c = b.extend({}, d);
			a == f.events.JWPLAYER_FULLSCREEN && !c.fullscreen ? (c.fullscreen = "true" == c.message ? !0 : !1, delete c.message) : "object" == typeof c.data ? (c = b.extend(c, c.data), delete c.data) : "object" == typeof c.metadata && b.deepReplaceKeyName(c.metadata, ["__dot__", "__spc__", "__dsh__", "__default__"], [".", " ", "-", "default"]);
		...
		f.eventdispatcher = function (f,
				...
				if (c[b]) {
					try {
						for (var h = 0; h < c[b].length; h++)
							if (c[b][h].listener.toString() == e.toString()) {
								c[b].splice(h, 1);
								break
							}
					} catch (f) {
				...
				if (d) {
					try {
						for (var c = 0; c < b.length; c++)
							if (b[c].listener.toString() == d.toString()) {
								b.splice(c, 1);
								break
							}
					} catch (e) {
			...
			this.sendEvent = function (d, n) {
				...
				e && a.log(d, n);
				if ("undefined" != a.typeOf(c[d]))
					for (var h = 0; h < c[d].length; h++) {
						try {
							c[d][h].listener(n)
						} catch (r) {
		...
		f.plugin = function (e) {
			...
			var n = a.loaderstatus.NEW,
				h, r, p, q, j = new k.eventdispatcher;
			a.extend(this, j);
			this.load = function () {
				if (n == a.loaderstatus.NEW)
					if (0 < e.lastIndexOf(".swf")) h = e, n = a.loaderstatus.COMPLETE, j.sendEvent(k.COMPLETE);
	...
	function (f) {
		...
		f.playlist = function (a) {
			...
			if ("array" == f.utils.typeOf(a))
				for (var e = 0; e < a.length; e++) k.push(new f.playlist.item(a[e]));
			else k.push(new f.playlist.item(a));
			return k
		}
	}(jwplayer), function (f) {
		var a = f.item = function (k) {
			var e = jwplayer.utils,
				c = e.extend({}, a.defaults, k);
		...
		var a = f.utils,
			...
			d = f.embed = function (n) {
				...
				var m = new d.config(n.config),
					l, t, u, w = m.width,
					z = m.height,
					B = "Error loading player: ",
					y = f.plugins.loadPlugins(n.id, m.plugins),
					x = null;
				...
				y.addEventListener(k.COMPLETE, function () {
					...
					if ("array" == a.typeOf(m.playlist) && 2 > m.playlist.length && (0 == m.playlist.length || !m.playlist[0].sources ||
						...
						for (var f = 0; f < m.modes.length; f++)
							if (m.modes[f].type && d[m.modes[f].type]) {
								var j = a.extend({}, m),
									D = new d[m.modes[f].type](l, m.modes[f], j, y, n);
		...
		function a(a) {
			if (a.playlist)
				for (var d = 0; d < a.playlist.length; d++) a.playlist[d] = new c(a.playlist[d]);
			else {
				var f = {};
		...
		(f.embed.config = function (b) {
			...
			if ("string" != typeof c || !e.exists(c)) d = 0;
				...
				var h = c.indexOf(":"); - 1 == h ? d = 0 : (d = parseFloat(c.substr(0, h)), c = parseFloat(c.substr(h + 1)), d = 0 >= d || 0 >= c ? 0 : 100 * (c / d) + "%")
		...
		f.embed.download = function (e, c, b) {
			...
			var h = a.extend({}, c),
				r = h.width ? h.width : 480,
				p = h.height ? h.height : 320,
				q;
			c = c.logo ? c.logo : {
				prefix: a.repo(),
				file: "logo.png",
			...
			if (h && h.length) {
				...
				for (h = 0; h < q.length; h++) {
					var u = q[h],
						w = u.type ? u.type : a.extensionmap.extType(a.extension(u.file));
		...
		var a = f.embed,
			...
			e = k.extend(function (c) {
				...
				var p = k.repo(),
					q = k.extend({}, f.defaults),
					j = k.extend({}, q, c.config),
					g = c.config,
					m = g.sitecatalyst,
					l = j.plugins,
					t = j.analytics,
			...
			d = f.api = function (a) {
				...
				g.resize = function (a, c) {
					if ("flash" != g.renderingMode) {
						...
						var d = b.getElementById(g.id + "_wrapper"),
							e = b.getElementById(g.id + "_aspect");
		...
		f.api.instream = function (c, b, d, f) {
			...
			this.dispatchEvent = function (a, b) {
				if (j[a])
					for (var c = k.translateEventResponse(a, b[1]), d = 0; d < j[a].length; d++) "function" == typeof j[a][d] && j[a][d].call(this, c)
			};
