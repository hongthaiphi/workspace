        ...
        connection.query('UPDATE camera set lastupdatetime=?  where device_id=?',
            [currentTime, server_format], function (err, result) {
                ...
                if (result.changedRows > 0) {
                    var json = { error: 0, message: 'Cập nhật thành công ' + server_format };
                } else {
                    var json = {
                        'error': '1',
        ...
        connection.query('select * from servers where device_id=?',
            [device_id_server], function (err, result) {
                ...
                if (result.length == 0) {
                    ...
                    var currentTime = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
                    connection.query('update server_module set status_last_update = ?, module_status=? where server_id=? and module_id=?',
                        [currentTime, module_status, server_id, module_id],
                        function (err, result) {
                            connection.release();
                            // connection.release();
                            if (err) {
                        ...
                        [device_id_camera], function (err, result) {
                            if (err) {
                                ...
                                if (result.length > 0) {
                                    ...
                                    json = { error: 0, message: 'Insert thanh cong' }
                                }
                            }
                            console.log('server_id ' + server_id);
                            console.log(json);
...
var getcamera = "SELECT ca.device_id,ca.ipcamera, tc.resolution, CASE  WHEN (ca.username IS NULL && ca.password IS NULL)||(ca.username='' && ca.password='') THEN"
    ...
    + " INNER JOIN type_camera tc on ca.type_camera_id=tc.id "
    + " WHERE se.device_id= ? ";
...
+ " md.name, unix_timestamp(mv.lastupdate) lastupdate FROM server_module sm ,module_version mv,module md" 
...
function getLastVersion(req, res, connection) {
    ...
    connection.query(getLastVersionQuery, [device_id_server], function (err, result) {
        ...
        if (err) {
            ...
            var listCam = '';
            for (cam in result) {
                listCam += result[cam].module_id + ';install;' + result[cam].name + ';' + result[cam].version + ';' + result[cam].lastupdate + ';' + result[cam].path_file + '\n';
            }
...
function updateInstallRemoveModule(req, res, connection) {
    ...
    for (var i in module_array) {
        ...
        connection.query('select * from servers where device_id=?',
            [device_id_server], function (err, result) {
                ...
                if (result.length == 0) {
                    ...
                    var server_id = result[0].id;
                    console.log('found server_id: '+server_id);
                    //chinh thuc xu li
                    async.parallel([
                        function (callback) {
                            getModuleNeedInstall(module_id_list, server_id, callback, connection);
                        },
                        function (callback) {
                            getModuleNeedUpdate(module_id_list, module_version_list, server_id, callback, connection);
                            // getModuleNeedRemove([2,3], callback);
                        },
                        function (callback) {
                            getModuleNeedRemove(module_id_list, server_id, callback, connection);
                        }
                    ],
                        // optional callback
        ...
        res.json({error: "000", message: "Thieu tham so module_id"});
    }
}
...
function getModuleNeedRemove(module_id_list, server_id, callback, connection) {
    var db_log = '';
    console.log('server_id server_id ' + server_id);
    async.forEachOf(module_id_list, function (moduleId, key, callback) {

        // Perform operation on file here.
        console.log('getModuleNeedRemove module_id ' + moduleId);
        console.log('getModuleNeedRemove server_id ' + server_id);
        // selectDB(res, connection, device_id_server, idCamera, sqlSelect, ipList[key]);
        connection.query("select * from module_view where module_id=? and server_id=? ", [moduleId, server_id], function (err, result) {
...
function doUpdateServerStatistic(err, result, connection, res, req) {
    if (err) {
        connection.release();
        res.json({ error: 1, message: err });
    }
...
function updateServerStatistic(req, res, connection) {
    ...
    if (device_id_server !== 'undefined' && device_id_server) {
        connection.query('select * from servers where device_id=?',
            [device_id_server], function (err, result) {
                doUpdateServerStatistic(err, result, connection, res, req);
            }
        )
    } else {
        connection.release();
