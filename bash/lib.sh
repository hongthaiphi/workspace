...
&token=AAGGl69284qM_H4i7fsxCXOkhSEe0OI7i_U&msg=STOP.$CHANNEL_NAME.$HOSTNAME"
				fi
			else
				echo "No channel need to STOP!"
			fi
...
startChannel2(){
	...
	if [ $CHANNELLIST ]; then
		...
		wget -qO /dev/null "http://cp.cam9.tv:3001/toTelegram?sendTo=-128035466
&token=AAGGl69284qM_H4i7fsxCXOkhSEe0OI7i_U&msg=Start.$CHANNELLIST.$HOSTNAME"
	else
...
infoChannel2(){
	ACTION=$1
	DOMAIN=$(/bin/cat /home/namnd/domain.conf | /usr/bin/awk -F'=' '{print $2}')
	INFO_CHANNEL=$(wget -qO /dev/null "http://$DOMAIN/admin/api/channel/?action=startstopchannel&hash=f21fd99bceccdf571a74b450f409b266f82718b630658179c87017618418fc84&secret_id=1" -O- | /usr/bin/tail -n 1 | /usr/bin/head -n 1)
	CHANNEL=$(echo "$INFO_CHANNEL" | /usr/bin/awk -F'info-' '{print $2}')
	...
	for i in "${ADDR[@]}"; do
	    ...
	    IP_ADDR=$(echo "$i" | /usr/bin/awk -F'-' '{print $5}')

	    if [ "$HOSTNAME" = "$CHANNEL_NVR" ]; then
	    	if (/bin/ping -w1 -n -c1 $IP_ADDR &> /dev/null); then
