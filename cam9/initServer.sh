...
doInit()
{
	...
	while IFS='' read -r i || [[ -n "$i" ]]; do
		...
		if [ $MODULE_ID == 1 ]; then
			wget $MODULE_URL -O /home/sysadmin/cam9/UpdateServerModule.sh
			chmod +x /home/sysadmin/cam9/UpdateServerModule.sh
		fi
		
		if [ $MODULE_ID == 2 ]; then
			wget $MODULE_URL -O /home/sysadmin/cam9/UpdateServerStatus.sh
			chmod +x /home/sysadmin/cam9/UpdateServerStatus.sh
		fi
		
	done <<< "$(cat $CAM9/run/updateServerModules.version.txt)"
	echo "finished init server"
}
#main
CAM9="/home/sysadmin/cam9"
...
doInit
touch /home/sysadmin/cam9/run/client.version.txt
