...
FILE_NAME="$HOME_PATH/cam9/log/server_statistic.log"
PREFIX="[updateServerStatistic.sh][$0]"
...
function updateServerStatistic(){
	...
	wget -qO /dev/null "http://cp.cam9.tv:3000/module?action=updateServerStatistic&device_id_server=$DEVICE_ID&ram=$getRAMInfo&free_space=$checkDisk&cpu_temp=$checkTemperature&cpu_load=$cpuLoad"
	writeAllLog $FILE_NAME $PREFIX "http://cp.cam9.tv:3000/module?action=updateServerStatistic&device_id_server=$DEVICE_ID&ram=$getRAMInfo&free_space=$checkDisk&cpu_temp=$checkTemperature&cpu_load=$cpuLoad"
}
