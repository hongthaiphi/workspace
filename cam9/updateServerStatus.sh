...
PREFIX="[updateServerStatus.sh][$0]"
...
function runAllModule(){
	while IFS='' read -r i || [[ -n "$i" ]]; do
		...
		then
			...
			writeAllLog $FILE_NAME $PREFIX "Module $MODULE_ID is not running"
			RUN_COMMAND="/home/sysadmin/cam9/source/$MODULE_ID/${array[last_index-1]} &> /dev/null sleep 1 & echo \$! > /home/sysadmin/cam9/run/$MODULE_ID.pid &"
